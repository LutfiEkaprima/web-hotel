...
if (isset($_POST['pesan'])) {
    // Validasi nomor identitas harus 16 angka
    if (strlen($nomor_identitas) != 16 || !is_numeric($nomor_identitas)) {
        echo "<script>alert('Nomor identitas harus berisi 16 angka.'); window.history.back();</script>";
        exit();
    }
...
<body style="background-color: #e9ecef;">
    <div class="container my-5">
        ...
        <div class="container container-form">
            <h2 class="text-center form-header">Pesanan</h2>
            <form action="pesanan.php" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" id="option1" name="jk" value="Laki-laki" <?= $jenis_kelamin == 'Laki-laki' ? 'checked' : '' ?> required>
                        <label class="form-check-label" for="option1">Laki-laki</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" id="option2" name="jk" value="Perempuan" <?= $jenis_kelamin == 'Perempuan' ? 'checked' : '' ?> required>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="nomor_identitas" id="nomor_identitas" class="form-control" value="<?= htmlspecialchars($nomor_identitas) ?>" minlength="16" maxlength="16" pattern="\d{16}" required>
                </div>

                <div class="mb-3">
                    <label class="form-label" for="kamar">Tipe Kamar</label>
                    <select name="kamar" id="kamar" class="form-select" onchange="this.form.submit()" required>
                ...
                <div class="mb-3">
                    <label class="form-label" for="harga">Harga per Malam</label>
                ...
                <div class="mb-3">
                    <label class="form-label" for="tanggal_pesanan">Tanggal Pesanan</label>
                ...
                <div class="mb-3">
                    <label class="form-label" for="durasi">Durasi (Hari)</label>
                    <input type="number" class="form-control" name="durasi" id="durasi" value="<?= htmlspecialchars($durasi) ?>" required>
                </div>

                <div class="mb-3">
                    <label class="form-check-label" for="makan">Termasuk Breakfast</label>
                    <input type="checkbox" class="form-check-input" name="makan" id="makan" <?= $termasuk_makan ? 'checked' : '' ?>>
                </div>
                ...
                <div class="d-flex gap-3">
                    <button type="submit" name="hitung" class="btn btn-secondary btn-custom">Hitung</button>
                    <button type="submit" name="pesan" class="btn btn-primary btn-custom">Pesan</button>
                </div>
            </form>
        </div>
    </div>
